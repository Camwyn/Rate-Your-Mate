<?php
    ...
    if(isset($session->currproj)||isset($_SESSION['currproj'])){$project=$session->currproj=$_SESSION['currproj'];}else{$project=null;}
    if(isset($session->currgroup)||isset($_SESSION['currgroup'])){$group=$session->currgroup=$_SESSION['currgroup'];}else{$group=null;}
    if(!is_null($project)&&$project!='Chose one...'){
        ...
        if($session->isInstructor()){//for instructors/admins
            ...
            */
            try{
                $sth=$database->connection->prepare("SELECT count(UID) AS count FROM Groups WHERE GID=:gid AND UID NOT IN (SELECT UID FROM Contract_Flags WHERE Flag=1)");
                $sth->execute(array(":gid"=>$group));
                while($row=$sth->fetch(PDO::FETCH_ASSOC)){
                    $count=$row['count'];
                }
            }catch(Exception $e){
