<?
    ...
    *
    * Originally ritten by: Jpmaster77 a.k.a. The Grandmaster of C++ (GMC)
    * Customized and altered by Stephe J Page (stephenjpage@gmail.com) 
    * for use in the Rate-Your-Mate system
    */
    ...
    class Session{
        ...
        */
        function checkLogin(){
            ...
            if(isset($_COOKIE['cookname']) && isset($_COOKIE['cookid'])){
                ...
                $this->SESID   = $_SESSION['SESID']   = $_COOKIE['cookid'];
            }
            ...
            $_SESSION['username'] != ""){
                ...
                if($database->confirmUserid($_SESSION['username'], $_SESSION['SESID']) != 0){
        ...
        */
        function register($subuser, $subpass, $subemail){
            ...
            if(!$subuser || strlen($subuser = trim($subuser)) == 0){
                $form->setError($field, "* Username not entered");
            }else{/* Spruce up username, check length */
                $subuser = stripslashes($subuser);
                if(strlen($subuser) < 5){
                    $form->setError($field, "* Username below 5 characters");
                }else if(strlen($subuser) > 30){
                        $form->setError($field, "* Username above 30 characters");
                    }else if(!preg_match("/^([0-9a-z])+$/i", $subuser)){/* Check if username is not alphanumeric */
                            $form->setError($field, "* Username not alphanumeric");
                        }else if(strcasecmp($subuser, "") == 0){/* Check if username is reserved */
                                $form->setError($field, "* Username reserved word");
                            }else if($database->usernameTaken($subuser)){/* Check if username is already in use */
            ...
            if(!$subpass){
                $form->setError($field, "* Password not entered");
            }else{/* Spruce up password and check length*/
            ...
            }else{/* No errors, add the new account */
        ...
        * the md5 hash of it to be used as a UID.
        */
